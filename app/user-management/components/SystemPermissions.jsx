...
function IcoDeptWrite() {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '20px', height: '20px', background: '#1a4d8f', borderRadius: '4px', flexShrink: 0 }}>
  ...
  const flatRows = useMemo(() => {
    ...
    SECTIONS.forEach(sec => {
      ...
      if (!collapsed[sec.name]) {
        sec.rows.forEach(r => rows.push({ ...r, type: r.type ?? 'row', subrow: r.subrow ?? false }));
      }
    });
    return rows;
  }, [collapsed]);
          ...
          <thead>
            <tr style={{ borderBottom: '2px solid #c8d1dc' }}>
              <th style={{
                ...
                Section / Sub-Section / Page / Functionality
              </th>
              {ROLES.map(role => (
                <th key={role} style={{
          ...
          <tbody>
            {visible.map((row, i) => {
              if (row.type === 'subsection') {
                return (
                  <tr key={`sub-${row.name}-${i}`} style={{ background: '#dce6f5' }}>
                    <td
              ...
              return (
                <tr key={`${row.name}-${i}`} style={{ background: bg, borderBottom: '1px solid #e2e8f0' }}>
                  <td style={{ ...sticky, padding: '7px 14px', color: '#1e293b', background: bg, borderRight: '1px solid #e2e8f0', whiteSpace: 'nowrap', ...(row.subrow && { borderLeft: '3px solid #1a4d8f' }) }}>
                    {row.indent ? (
                      ...
                      <span style={{ paddingLeft: '14px', color: '#3a4a5c' }}>{row.name}</span>
                    ) : row.name}
                  </td>
